...
{
    //create a print method, put this inside each sorting function so that each step is printed out
		...
		for (position =  data.length - 1; position >= 0; position--)
		{
			for (scan = 0; scan <= position - 1; scan++)
			{
				if (data[scan].compareTo(data[scan + 1]) > 0)
					...
					printArray(data); //HERE FOR NOW.  
				//printArray(data); //here?
			}
			//printArray(data); //here?
		}
	}
	 ...
	 */
	private static <T extends Comparable<T>> 
	void quickSort(T[] data, int min, int max) //DID NOT PUT THE PRINT ARRAY METHOD IN HERE. 
	{
		if (min < max)
		{
			// create partitions
